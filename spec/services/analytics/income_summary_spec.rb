...
RSpec.describe Analytics::IncomeSummary do
    ...
    context "when an oxidant is selected" do
      ...
      before do
        ...
        create(
          ...
          oxidant: [
            {
              "formula_product_id" => oxidant_product.id,
              "amount" => 20,
              "price" => 2
            }
          ]
    ...
    let(:filters) do
      {
        ...
        formula_product_ids: [ color_product.id ],
        care_product_ids: [ care_product.id ]
      }
    end
      ...
      expect(summary.care_product_options).to contain_exactly(
        {
          ...
          label: "Londa Visible Repair Mask"
        }
      )
    end

    it "builds color brands from available color options" do
      expect(summary.formula_color_brands).to eq([ "Londa" ])
    end

    it "builds care product brands from available care product options" do
      expect(summary.care_product_brands).to eq([ care_product.brand ])
    end

    it "builds care product categories from available care product options" do
  ...
  describe "income breakdown" do
    ...
    it "matches the care product breakdown with care products income" do
      breakdown_total =
        summary.care_product_income.sum { |item| item[:amount] }
